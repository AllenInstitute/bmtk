...
def test_find_sections():
    ...
    secs, sec_probs = morph.find_sections(target_sections=['axon'], distance_range=[0.0, 10.0e20])
    for sec in secs:
        assert (morph.seg_prop['type'][sec] == 2)

    secs, sec_probs = morph.find_sections(target_sections=['soma'], distance_range=[300.0, 500.0])
    assert(len(secs) == 0)
    assert(len(sec_probs) == 0)

    secs, sec_probs = morph.find_sections(target_sections=['soma'], distance_range=[0.0, 100.0])
