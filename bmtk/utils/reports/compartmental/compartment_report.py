...
class CompartmentReport(object):
    def __init__(self, file_name, tmp_dir, variables, default_population='pop_na', buffer_data=True, mpi_rank=0, mpi_size=1):
        self._file_name = file_name
        self._tmp_dir = tmp_dir
    ...
    def initialize(self, n_steps, buffer_size=0):
        for pop_grp in self._pop_tables.items():
            pop_grp.initialize(n_steps=n_steps, buffer_size=buffer_size)

    def record_cell(self, node_id, var_name, seg_vals, tstep, population=None):
        pop_str = population or self._default_pop
        pop_grp = self._pop_tables[pop_str]
        pop_grp.record_cell(node_id=node_id, var_name=var_name, seg_vals=seg_vals, tstep=tstep)

    def record_cell_block(self, node_id, var_name, seg_vals, population=None):
        pass
    ...
    def _build_or_fetch_pop(self, population):
        if population in self._pop_tables:
            ...
            pop_grp = _PopGroup(self._file_name, self._tmp_dir, self._variables)
...
class CompartmentReportParallel(CompartmentReport):
    """
