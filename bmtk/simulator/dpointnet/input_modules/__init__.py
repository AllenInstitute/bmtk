...
class InputModules:
    ...
    def get_module(self, input_type, module_name):
        if module_name not in self._input_module_classes[input_type]:
            raise ValueError(f'Could not find module "{module_name}" for input type "{input_type}"')
    ...
    def to_dataframe(self):
        return pd.DataFrame(self.to_list(), columns=['input_type', 'module_name', 'module_class'])
