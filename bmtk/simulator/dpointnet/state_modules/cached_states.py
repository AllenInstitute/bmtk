...
class CachedInitState:
    def __init__(self, file_paths, file_type='mixed', **kwargs):
        ...
        if len(self.all_cache_files) == 0:
            raise ValueError(f'{self.__class__} Could not find any cache files using file_paths="{file_paths}"')
        ...
        if file_type is None or file_type in ['mixed', '']:
            ...
            raise ValueError(f'{self.__class__}: Invalid file_type option "{file_type}". Available options [mixed, npz, pkl, h5].')
    ...
    def _load_npz(file_path, rnn):
        ...
        state_vals = []
        _, state_names = rnn.cell.zero_state(batch_size=rnn.batch_size, dtype=rnn.dtype, with_names=True)
