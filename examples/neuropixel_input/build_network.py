...
def build_lgn(visp):
    ...
    lgn.add_edges(
        target=visp.nodes(ei='e', model_type='biophysical'), source=lgn.nodes(),
    ...
    lgn.add_edges(
        target=visp.nodes(ei='i', model_type='biophysical'), source=lgn.nodes(),
        connection_rule=connect_lgn,
        iterator='one_to_all',
        dynamics_params='AMPA_ExcToInh.json',
        model_template='Exp2Syn',
    ...
    lgn.add_edges(
        target=visp.nodes(model_type='point_neuron'), source=lgn.nodes(),
        connection_rule=connect_lgn,
        iterator='one_to_all',
        dynamics_params='instantaneousExc.json',
        delay=2.0,
...
def build_visl(visp):
    ...
    # Targets all biophysical excitatory cells
    visal.add_edges(
        target=visp.nodes(ei='e', model_type='biophysical'), source=visal.nodes(),
    ...
    # Targets all biophysical inhibitory cells
    visal.add_edges(
        target=visp.nodes(ei='i', model_type='biophysical'), source=visal.nodes(),
        connection_rule=lambda *_: np.random.uniform(0, 20),
        dynamics_params='AMPA_ExcToInh.json',
        model_template='Exp2Syn',
        delay=2.0,
    ...
    # Targets all intfire1 cells (exc and inh)
    visal.add_edges(
        target=visp.nodes(model_type='point_neuron'), source=visal.nodes(),
        connection_rule=lambda *_: np.random.uniform(0, 20),
        dynamics_params='instantaneousExc.json',
        delay=2.0,
        syn_weight=0.045
