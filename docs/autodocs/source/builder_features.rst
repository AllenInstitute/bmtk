...
Iterating and Filter Nodes from a Network using ``nodes()`` method
------------------------------------------------------------------
...
``nodes()`` parameters. For example if we want to find only those cells that are **excitatory point** neuron cells from
the example above
...
Simular to the ``nodes()`` method, the :py:meth:`NetworkBuilder.edges() <bmtk.builder.network.Network.edges>` method
...
The following small network will display the following like the following (**note that the order of edges 
may be different**)
...
.. important::
    ...
    and memory. And if you call ``add_edges()`` or ``add_nodes()`` after it will need to rebuild the network. 

    As such it is highly recommend to not call ``edge()`` iterator until after all calls to ``add_nodes()`` and 
...
At the beggining of each simulation *BioNet* will see the **target_section** and **distance_range** parameters and use 
them to find every morphology segment that matches, choose one at random, and create a synapse.
...
reproducability of network results. 
...
you can use the ``SWCReader`` class inside your script to access information about availabe sections 
...
By default when you when have a population network with name "NETNAME" and call ``NetworkBuilder.save()`` or 
``NetworkBuilder.save_nodes()`` the Builder will by default save it to file names *NETNAME_nodes.h5* and 
...
the target_node_id, source_node_id, or edge_type_id. By default BMTK Network Builder will create indices for 
...
To save an hdf5 file with a compression use the **compression** option (**note** that the options for compression may 
vary depending on machine on hdf5/h5py libraries installed).
...
the script with the approiate command for your system (eg. ``mpirun``, ``mpiexec``, ``srun``, etc).
...
The resulting SONATA network file will be similar to as if run in single core python. There is no need for the user 
...
The simplest solution is to make sure to hard-code the random seed. This will better ensure that each take a random 
numbers are generated they produce the same values across all ranks. 
...
Another option is to use the **mpi4py** ``Bcast`` function to generate values to generate inputs on one rank then 
