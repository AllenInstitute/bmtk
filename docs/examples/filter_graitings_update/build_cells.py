        ...
        for fname in os.listdir(basepath):
            ...
            if os.path.isfile(os.path.join(basepath, fname)):
                ...
                if tf_str in pkl_savename.split('_') and pkl_savename.find(cell_type) >= 0 and pkl_savename.find('.pkl') >= 0:
                    ...
                    # print 'found {}'.format(os.path.join(basepath, fname))
...
for fname in glob.glob('optimized_params/*.pkl'):
    dynamics_params = pickle.load(open(fname, 'rb'))
    for k, v in dynamics_params.items():
        if isinstance(v, np.ndarray):
            dynamics_params[k] = v.tolist()
        #print type(v), v
    #exit()
    json_path = os.path.join('../filter_components/model_templates', '{}.json'.format(fname[17:][:-4]))
    with open(json_path, 'w') as fp:
        json.dump(dynamics_params, fp, indent=2)
'''
