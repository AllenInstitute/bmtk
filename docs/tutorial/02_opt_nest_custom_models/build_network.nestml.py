...
lif_models = {
    'LIF_exc': {
        ...
        'model_template': 'nestml:custom_izh_neuron_nestml',
        'dynamics_params': 'custom_izh_neuron_exc.json'
    },
    'LIF_inh': {
        'N': 5,
        'ei': 'i',
        ...
        'model_type': 'point_neuron',
        'model_template': 'nestml:custom_izh_neuron_nestml',
        'dynamics_params': 'custom_izh_neuron_inh.json'
    }
}
...
def build_cortex_network():
    cortex = NetworkBuilder('izh_network')
    ...
    cortex.build()
    cortex.save(output_dir='network_nestml')

    return cortex


def build_thalamus_network(cortex):
    thalamus = NetworkBuilder('virt_exc')
    thalamus.add_nodes(**input_network_model['input_network'])

    thalamus.add_edges(
        target=cortex.nodes(),
        connection_rule=random_connections,
        connection_params={'p': 0.8},
        syn_weight=8.2,
        delay=1.5,
    ...
    thalamus.build()
    thalamus.save(output_dir='network_nestml')
